	...
	public class FlowchartWindow : EditorWindow
	{
		...
		protected virtual void DrawWindow(int windowId)
		{
			...
			if (Event.current.type == EventType.ContextClick)
			{
				...
				menu.AddItem(new GUIContent ("Duplicate"), false, (obj) => { DuplicateBlock(obj); }, block);
				menu.AddItem(new GUIContent ("Delete"), false, DeleteBlock, block);

                var subMenuName = "Move to flowchart";
                foreach (var anotherFlowchart in FindAllFlowcharts())
                {
                    if(anotherFlowchart == flowchart)
                    {
                        continue;
                    }

                    var targetFlowchart = anotherFlowchart;

                    var subMenuItemName = string.Format("{0}/{1}", subMenuName, targetFlowchart.gameObject.name);
                    menu.AddItem(new GUIContent(subMenuItemName), false, (obj) => {
                        DuplicateBlock(obj, targetFlowchart);
                        DeleteBlock(obj);
                    }, block);
                }
        ...
        private Flowchart[] FindAllFlowcharts()
        {
            Flowchart[] flowcharts = (Flowchart[])FindObjectsOfType(typeof(Flowchart));
		...
		protected static void DuplicateBlock(object obj, Flowchart targetFlowchart = null)
		{
            Flowchart flowchart = targetFlowchart ?? GetFlowchart();
            Block block = obj as Block;
