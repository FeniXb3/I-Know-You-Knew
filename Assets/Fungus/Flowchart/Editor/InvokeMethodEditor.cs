...
namespace Fungus
{
	...
	public class InvokeMethodEditor : CommandEditor
	{
	    ...
	    public override void DrawCommandGUI()
	    {
            ...
            ShowTargetObject(objSerializedTarget);

            if (targetMethod.GetGameObject() == null)
                return;
            
	        string component = ShowComponents(objSerializedTarget, targetMethod.GetGameObject());

	        // show component methods if selected
	        if (!string.IsNullOrEmpty(component))
	        {
	            var method = ShowMethods(objSerializedTarget, targetMethod.GetGameObject(), component);

	            // show method parameters if selected
	            if (method != null)
	            {
	                objSerializedTarget.ApplyModifiedProperties();
	                ShowParameters(objSerializedTarget, targetMethod.GetGameObject(), method);
        ...
        private void ShowTargetObject(SerializedObject objTarget)
        {
            var targetObjectProp = objTarget.FindProperty("targetObject");
            var variableType = typeof(GameObject);
            string labelFormat = targetObjectProp.displayName;

            EditorGUILayout.BeginVertical();
            DrawVariable(targetObjectProp, variableType, labelFormat);
        ...
        private void ShowParameters(SerializedObject objTarget, GameObject gameObject, MethodInfo method)
	    {
	        ...
	        if (objParams.Length > 0)
	        {
	            ...
	            for (int i = 0; i < objParams.Length; i++)
                {
                    ...
                    var parameterType = objParam.ParameterType;

                    DrawVariable(objItemProp, parameterType, labelFormat);
                }
                EditorGUILayout.EndVertical();
        ...
        private void DrawVariable(SerializedProperty objItemProp, Type variableType, string labelFormat)
        {
            ...
            serValueTypeAssemblynameProp.stringValue = variableType.AssemblyQualifiedName;
            serValueTypeFullnameProp.stringValue = variableType.FullName;

            bool isDrawn = true;

            if (string.IsNullOrEmpty(serVariableKeyProp.stringValue))
            {
                isDrawn = DrawTypedPropertyInput(labelFormat, serObjValueProp, variableType);
            }

            if (isDrawn)
            {
                var vars = GetFungusVariablesByType(targetMethod.GetFlowchart().variables, variableType);
            ...
            else
            {
                ...
                EditorGUILayout.LabelField(new GUIContent(variableType.Name + " cannot be drawn, don´t use this method in the flowchart."), style);
	    ...
	    private bool DrawTypedPropertyInput(string label, SerializedProperty objProperty, Type type)
	    {
            ...
            {
                objectValue = objProperty.FindPropertyRelative("animationCurveValue");
                objectValue.animationCurveValue = EditorGUILayout.CurveField(new GUIContent(label), objectValue.animationCurveValue);
